...
export class SEOAnalyzer {
  ...
  initializeAI() {
    ...
    if (process.env.GROK_API_KEY) {
      this.aiModels.grok = new ChatOpenAI({
    ...
    if (process.env.OPENAI_API_KEY) {
      this.aiModels.openai = new ChatOpenAI({
      ...
      if (this.aiModels[aiType]) {
        try {
          ...
          if (aiType === 'gemini') {
            ...
            const response = await this.aiModels[aiType].invoke([
              { role: "system", content: "Você é um especialista em SEO e análise de sites." },
