...
import { getRecommendation } from '../../api/product-recommender.js';
import { isHotLead, notifyHotLead, checkNotificationConfig } from '../utils/notifications.js';
...
export class SaraAIv2 {
  constructor() {
    ...
    // Mostra configuração de notificações
  ...
  async processMessage(userMessage, userInfo = {}, chatHistory = []) {
    try {
      ...
      // 2. Obtém recomendação inteligente de produto (NOVO!)
      const productRecommendation = getRecommendation(
        userMessage,
        chatHistory.map(msg => msg.content || '')
      );

      console.log(`🎯 Produto recomendado: ${productRecommendation.recommendation?.primary_product?.name || 'Qualificação necessária'} | Tipo: ${productRecommendation.recommendation?.recommendation_type}`);

      // 3. Constrói contexto dinâmico estruturado (agora com recomendação de produto)
      const dynamicContext = buildDynamicContext({
        ...
        productRecommendation: productRecommendation // Injeta recomendação
      });
      ...
      // 7.5 🔥 NOVO: Verifica se é lead quente e notifica
      if (isHotLead(leadScore, parsed.dados_extraidos)) {
        console.log('🔥 Lead quente detectado! Enviando notificações...');
        try {
          await notifyHotLead(
            {
              leadScore,
