 ...
 *
 * Prioridade:
 * 1. Claude 3 Haiku - Honesto, segue instruções complexas, JSON consistente (principal)
 * 2. Gemini 2.0 Flash - Fallback rápido e econômico
...
export class MultiAPIManagerV2 {
   ...
   */
  initializeModels() {
    // 🥇 PRIORIDADE 1: Claude 3 Haiku (Anthropic)
    // Melhor para: venda consultiva, honestidade, instruções complexas, JSON consistente
    ...
    if (claudeApiKey) {
      try {
        this.models.claude = new ChatAnthropic({
          ...
          temperature: 0.4,  // Baixo: mais consistente, menos alucinação, JSON correto
          maxTokens: 512,    // Sara deve ser concisa — respostas curtas
        });
        this.modelPriority.push('claude');
        console.log("✅ Claude 3 Haiku inicializado (Prioridade 1 — Principal)");
      } catch (error) {
        console.warn("⚠️ Erro ao inicializar Claude:", error.message);
      }
    }

    // 🥈 PRIORIDADE 2: Gemini 2.0 Flash (Google)
    // Melhor para: fallback rápido quando Claude não está disponível
    ...
    if (geminiApiKey) {
      try {
        ...
        console.log("✅ Gemini 2.0 Flash inicializado (Prioridade 2 — Fallback)");
      } catch (error) {
