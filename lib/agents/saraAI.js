...
class MultiAPIManager {
  ...
  getModel() {
    ...
    return {
      invoke: async () => ({ content: "Erro: Nenhuma chave de API configurada (Anthropic, Grok ou OpenAI)." })
    };
  }
}
...
export class SaraAI {
  ...
  async applySaraFilter(model, draftResponse, expertUsed, userInfo) {
    ...
    const systemPromptBase = `
    VOCÊ É A SARA: A IA Especialista em Vendas e Marketing da Ronald Digital.
  ...
  async useFallbackModel(systemPrompt, userMessage) {
    ...
    if (this.apiManager.models.openai) {
      try {
         const messages = [
          new SystemMessage(systemPrompt),
          new HumanMessage(userMessage)
        ];
        const res = await this.apiManager.models.openai.invoke(messages);
        return res.content;
      } catch(e) { console.error("Erro no Fallback OpenAI:", e.message); }
    }
